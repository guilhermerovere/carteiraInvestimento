...
test("login → carteira renderiza shell e resumo sem expor sessão", async ({ page }) => {
  ...
  await expect(page.getByText(/R\$ 125\.001\.(200|500),22/)).toBeVisible();
...
test("roles incompatíveis recebem acesso negado sem perder a sessão", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Administração Valore" })).toBeVisible();
  await page.context().clearCookies();
  await login(page);
  await page.goto("/admin");
  await expect(page.getByRole("heading", { name: "Acesso negado" })).toBeVisible();
...
test("preview e navegação abrem posições, transações e movimentações", async ({ page }) => {
  await login(page);
  await page.getByRole("link", { name: /Ver todos os ativos/ }).click();
  await expect(page).toHaveURL("/carteira/posicoes");
  await expect(page.getByRole("heading", { name: "Minhas posições" }).first()).toBeVisible();
  await expect(page.locator(".reference-table .table-asset", { hasText: "ACME3" })).toBeVisible();
...
test("refresh mantém a tela e atualiza somente o resumo", async ({ page }) => {
  await login(page);
  const button = page.getByRole("button", { name: "Atualizar" });
  await button.click();
  await expect(page.getByText("R$ 125.001.500,22")).toBeVisible();
...
test("login e admin continuam legíveis em Light e Dark", async ({ page }) => {
  ...
  await page.reload();
  await expect(page.locator("html")).toHaveClass(/dark/);
  await expect(page.getByRole("link", { name: "Gerenciar corretoras" })).toBeVisible();
});
