import { mapPage, mapPosition } from "@/server/finance/mappers";
...
export async function GET(request: Request) {
  try {
    ...
    const result = await financeBackendRequest("/api/v1/carteira/posicoes?page=" + page + "&size=" + size, {
      correlationId: safeCorrelationId(request.headers.get("x-correlation-id")),
    });
    return financeJson(mapPage(result.data, mapPosition), result.correlationId);
  } catch (error) {
