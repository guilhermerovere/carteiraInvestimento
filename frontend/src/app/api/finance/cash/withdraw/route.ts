...
export async function POST(request: Request) { const correlationId=safeCorrelationId(request.headers.get("x-correlation-id")); try { assertNoQuery(request); const key=assertMutationRequest(request); const body=cashBody(await strictJson(request)); const result=await financeBackendRequest("/api/v1/carteira/caixa/saque",{method:"POST",body,idempotencyKey:key,correlationId}); return financeJson(result.data,result.correlationId); } catch(error){return financeErrorResponse(error,correlationId);} }
