...
import { passwordBody } from "@/server/account/validation";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await financeBackendRequest("/api/v1/account/password", { method: "POST", body: passwordBody(await strictJson(request)), correlationId });
