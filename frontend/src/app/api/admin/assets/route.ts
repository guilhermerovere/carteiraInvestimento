...
import { adminListQuery, assetCreateBody } from "@/server/admin/validation";
...
export async function GET(request: Request) { const id = safeCorrelationId(request.headers.get("x-correlation-id")); try { const result = await financeBackendRequest("/api/v1/acoes" + adminListQuery(request, "assets"), { correlationId: id }); return financeJson(result.data, result.correlationId); } catch (error) { return financeErrorResponse(error, id); } }
export async function POST(request: Request) { const id = safeCorrelationId(request.headers.get("x-correlation-id")); try { assertNoQuery(request); if (!hasValidOrigin(request.headers.get("origin"))) throw new FinanceValidationError("Origem inválida."); const result = await financeBackendRequest("/api/v1/acoes", { method: "POST", body: assetCreateBody(await strictJson(request)), correlationId: id }); return financeJson(result.data, result.correlationId); } catch (error) { return financeErrorResponse(error, id); } }
