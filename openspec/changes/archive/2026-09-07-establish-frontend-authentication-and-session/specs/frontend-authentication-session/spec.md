...
`/inicio` SHALL exigir sessão confirmada e ROLE_USER; `/admin`, sessão confirmada e ROLE_ADMIN. proxy.ts SHALL ser somente pre-check de cookie ausente, usando apenas nome do cookie, caminhos protegidos e sanitização de returnTo. Ele MUST NOT importar cliente backend, resolvedor, JWT, Authorization, ProblemDetail upstream ou escrita de sessão, nem validar/decodificar token ou usar role como autoridade. A decisão final SHALL depender do usuário atual confirmado. Role insuficiente SHALL resultar em 403/acesso-negado e preservar sessão.

#### Scenario: Sessão ausente
- **WHEN** rota protegida é acessada sem cookie
- **THEN** proxy redireciona para login com returnTo interno seguro

#### Scenario: Role insuficiente
- **WHEN** usuário confirmado acessa rota de role não permitida
