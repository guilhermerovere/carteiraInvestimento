...
A raiz do repositório SHALL conter `.env` e `.env.example` com todas as variáveis necessárias para PostgreSQL, backend e frontend. O `.env.example` MUST usar valores de exemplo seguros, e nenhum dos arquivos MUST conter segredo real.
...
- **THEN** encontra as variáveis necessárias para iniciar os três serviços sem consultar configuração oculta
